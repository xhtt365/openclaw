...
export const useOfficeStore = create<OfficeState>((set, get) => {
  ...
  function applyChatUsageSnapshot(agentId: string, payload: GatewayChatEventPayload) {
    const usage = normalizeUsage(
      (payload.message as { usage?: unknown } | undefined)?.usage as
  ...
  function finishConversation(params: {
    ...
    if (hadToolFlow) {
      const taskLabel = shortenTaskLabel(prompt, "工具结果整理");
      setAgentStatusIfChanged(params.agentId, {
        action: "working",
        detail: `🧩 继续处理「${taskLabel}」…`,
        taskId: buildTaskBadge(`${params.agentId}-${startedAt}`),
      });
      get().addActivity(params.agentId, "回复输出完成，切到办公区继续处理", "executing");
      get().queueMove(params.agentId, "work");

      const timerId = window.setTimeout(() => {
        console.log(`[Office] tool-flow timer done: agent=${params.agentId}`);
        get().addActivity(params.agentId, "工具整理完成，返回休闲区", "done");
        get().queueMove(params.agentId, "lounge");
  ...
  function handleGatewayToolEvent(
    agentId: string,
    ...
    if (phase === "start") {
      // 对话过程里的工具调用先留在对话区，等完整输出结束后再决定是否切去办公区。
      const isConversationLive =
        activeConversationAgentIds.has(agentId) ||
        replyStartedAtByAgentId.has(agentId) ||
        replyingRunIds.has(runId);

      if (!isConversationLive && currentZone !== "work") {
    ...
    if (phase === "result") {
      ...
      get().addActivity(
        agentId,
        isError
          ? `${prettifyToolName(toolName) || "工具"} 执行失败`
          : `${prettifyToolName(toolName) || "工具"} 执行完成`,
        isError ? "error" : "done",
      );
    }
  }
