...
function mergeIdentity(agent: Agent, identity?: GatewayAgentIdentity) {
  ...
  const avatarValue =
    typeof identity.avatar === "string" && identity.avatar.trim() ? identity.avatar.trim() : ""
  const emoji =
    typeof identity.emoji === "string" && identity.emoji.trim()
      ? identity.emoji.trim()
      : avatarValue && !avatarValue.startsWith("data:image/") && !avatarValue.startsWith("http")
        ? avatarValue
        : agent.emoji
  ...
  return {
    ...
    emoji,
    avatarUrl:
      avatarValue.startsWith("data:image/") || avatarValue.startsWith("http")
        ? avatarValue
        : agent.avatarUrl,
  }
}
...
function resolveAvatarUrl(identity?: AgentsListIdentity) {
  ...
  if (typeof identity.avatar === "string" && identity.avatar.trim().startsWith("data:image/")) {
    return identity.avatar.trim()
  }
...
function resolveConfiguredModelLabel(
  ...
  const matched = entries.find(
    (entry) =>
      typeof entry?.id === "string" && normalizeAgentIdKey(entry.id) === normalizeAgentIdKey(agentId)
  )
...
function toSupportedModelApi(value: unknown) {
  ...
  return apiValue && isSupportedModelApi(apiValue) ? (apiValue as ModelApiProtocol) : undefined
}
...
function resolveModelApi(value: unknown) {
  ...
  return apiValue as ModelApiProtocol
}
...
function normalizeProviderApiKey(providerConfig: GatewayConfigRecord | null, parsedConfig: ParsedModelJSON) {
...
function upsertProviderModelEntry(targetProvider: GatewayConfigRecord, modelEntry: GatewayConfigRecord) {
  ...
  const modelIndex = nextProviderModels.findIndex(
    (model) => isConfigRecord(model) && toRequiredString(model.id).toLowerCase() === modelId.toLowerCase()
  )
...
function migrateProxyOpenAIModel(params: {
  ...
  const targetProvider = isConfigRecord(nextProviders[migratedProvider])
    ? structuredClone(nextProviders[migratedProvider] as GatewayConfigRecord)
    : {}
...
function ensureModelRuntimeConfigInConfig(params: {
  ...
  const nextProviderModels = Array.isArray(normalizedProvider.models) ? [...normalizedProvider.models] : []
  const modelIndex = nextProviderModels.findIndex(
    (model) => isConfigRecord(model) && toRequiredString(model.id).toLowerCase() === modelId.toLowerCase()
  )
  if (modelIndex === -1) {
  ...
  if (transport === "sse") {
    ...
    const existingMigratedProvider = isConfigRecord(nextProviders[migratedProvider])
      ? (nextProviders[migratedProvider] as GatewayConfigRecord)
      : null
...
export const useAgentStore = create<AgentState>((set, get) => ({
  ...
  fetchAgents: async () => {
    ...
    try {
      ...
      const defaultModelLabel = configSnapshot ? gateway.getDefaultModelLabel(configSnapshot) ?? null : null
      ...
      const identityFileResults = await Promise.allSettled(
        baseAgents
          .filter((agent) => (primedAgentFiles.get(agent.id) ?? []).some((file) => file.name === "IDENTITY.md"))
          .map(async (agent) => ({
            agentId: agent.id,
      ...
      const currentAgentModel =
        detailAgentId
          ? agents.find((agent) => agent.id === detailAgentId)?.modelName ?? defaultModelLabel ?? null
  ...
  fetchModels: async (force = false) => {
    ...
    try {
      ...
      const availableModels = mergeConfiguredModelsIntoAvailableModels(gatewayModels, configSnapshot)
  ...
  fetchAgentModel: async (agentId) => {
    ...
    const cachedModel =
      get().agents.find((agent) => agent.id === agentId)?.modelName ?? get().defaultModelLabel ?? null
    ...
    try {
      ...
      const modelName = resolveConfiguredModelLabel(configSnapshot, agentId, defaultModelLabel) ?? null
  ...
  ensureModelRuntimeConfig: async (agentId, modelRef) => {
    const resolvedModelRef =
      modelRef?.trim() ||
      get().agents.find((agent) => agent.id === agentId)?.modelName?.trim() ||
    ...
    if (syncResult.effectiveModelRef !== resolvedModelRef) {
      ...
      set((state) => ({
        currentAgentModel:
          state.currentAgentModel === resolvedModelRef ? syncResult.effectiveModelRef : state.currentAgentModel,
        agents: patchAgentById(state.agents, agentId, (agent) => ({
          ...agent,
          modelName:
  ...
  setAgentModel: async (agentId, model) => {
    ...
    try {
      ...
      set((state) => ({
        modelSaving: false,
        currentAgentModel: state.showDetailFor === agentId ? effectiveModelRef : state.currentAgentModel,
        agents: patchAgentById(state.agents, agentId, (agent) => ({
          ...agent,
  ...
  selectFile: async (name) => {
    ...
    try {
      ...
      const identityDetails = name === "IDENTITY.md" ? parseAgentIdentityContent(content) : undefined
  ...
  saveFile: async () => {
    ...
    try {
      ...
      set((state) => {
        const currentFile = state.agentFiles.get(showDetailFor)?.find((file) => file.name === activeFileName)
        return {
      ...
      console.log(`[Store] saveFile: ${ok ? "saved" : "skipped"} ${showDetailFor}/${activeFileName}`)
      return ok
    } catch (error) {
