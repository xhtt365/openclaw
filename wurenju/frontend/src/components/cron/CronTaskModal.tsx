...
export function CronTaskModal({
  ...
  const [form, setForm] = useState<CronTaskFormState>(() =>
    buildInitialState({
      job,
      lockedAgentId,
      defaultReplyMode,
  ...
  }, [defaultGroupId, defaultReplyMode, job, lockedAgentId, open]);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="flex max-h-[88vh] w-[min(760px,calc(100vw-2rem))] flex-col overflow-hidden rounded-[24px] border-[var(--modal-shell-border)] bg-[var(--modal-shell-bg)] p-0 text-[var(--color-text-primary)] shadow-[var(--modal-shell-shadow)] backdrop-blur-xl sm:max-w-[760px]">
        <div className="relative border-b border-[var(--modal-shell-border)] px-5 py-4">
          <div className="pointer-events-none absolute inset-x-0 top-0 h-24 bg-[radial-gradient(circle_at_top_left,var(--surface-brand-soft),transparent_56%),radial-gradient(circle_at_top_right,var(--brand-glow),transparent_32%)] opacity-90" />
          <div className="relative">
            <DialogHeader className="gap-2 text-left">
              <DialogTitle className="flex items-center gap-2 text-xl">
                <CalendarClock className="h-5 w-5 text-[var(--color-brand)]" />
                <span>{isEditMode ? "编辑定时任务" : "新建定时任务"}</span>
              </DialogTitle>
              <DialogDescription>
                {isEditMode
                  ? "修改任务频率、执行员工和回复落点。"
        ...
        <DialogFooter className="border-t border-[var(--modal-shell-border)] bg-[var(--surface-soft)] px-5 py-4">
          <Button
            type="button"
            variant="ghost"
