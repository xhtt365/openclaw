...
function getPreview(messages: ChatMessage[]) {
  const visibleMessages = messages.filter((message) => !message.isLoading && message.content.trim());
...
function EmployeeListInner({ selectedEmployeeId, onSelectEmployee }: EmployeeListProps) {
  ...
  const entries = agents.map((agent) => buildEmployee(agent, messagesByAgentId.get(agent.id) ?? []));
  const visibleEntries = entries.filter(({ employee, presentation }) => {
    ...
    const activeGroupLabel = FUNCTION_GROUPS.find((group) => group.id === activeGroupId)?.label ?? "";
    ...
    <>
      ...
      />
                            ...
                            >
                              ...
                              onClick={(event) => {
                                event.stopPropagation();
                                console.log(`[UI] 打开员工配置: ${employee.id}`);
                              }}
                              className="absolute right-3 top-1/2 flex h-8 w-8 -translate-y-1/2 translate-x-2 items-center justify-center rounded-full text-[var(--color-text-secondary)] opacity-0 transition-all duration-200 group-hover:translate-x-0 group-hover:opacity-100 hover:bg-[var(--color-bg-soft)] hover:text-[var(--color-text-primary)]"
                              aria-label={`打开 ${employee.name} 配置`}
                            >
                              <Settings2 className="h-4 w-4" />
                            </button>
