...
function AppLayoutInner() {
  ...
  return (
    <div className="flex h-screen min-h-0 overflow-hidden bg-[var(--color-bg-primary)]">
      <div className={cn("h-full shrink-0", isReady ? "panel-left-enter" : "")}>
        <EmployeeList
          selectedEmployeeId={selectedEmployee.id}
          onSelectEmployee={handleSelectEmployee}
        />
      </div>
      <div className={cn("h-full min-h-0 min-w-0 flex-1", isReady ? "panel-right-enter" : "")}>
        {showDetailFor !== null && selectedGroup === null && selectedArchive === null ? (
          <EmployeeDetailPage />
        ) : (
          <ChatArea employee={selectedEmployee} group={selectedGroup} archive={selectedArchive} />
        )}
      </div>
    </div>
  );
