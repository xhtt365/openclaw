...
const toastVariants = cva(
  "pointer-events-auto relative flex w-[min(420px,calc(100vw-2rem))] items-center justify-between space-x-4 overflow-hidden rounded-md border p-6 pr-8 shadow-lg",
  {
    variants: {
      variant: {
...
const ToastViewport = ({ className, ...props }: React.ComponentProps<"div">) => (
  <div
    aria-hidden
    className={cn(
      // Fix: 问题2 - Toast 改为跟随容器定位，只在右侧聊天区域或独立页面容器内显示。
      "pointer-events-none absolute inset-x-0 top-0 z-[99] flex flex-col items-end gap-3 p-4",
      className,
    )}
    {...props}
  />
