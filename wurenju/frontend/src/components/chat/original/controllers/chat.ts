// 复制自 openclaw 3.13 原版 ../../../ui/src/ui/controllers/chat.ts，用于二开定制
...
function shouldRenderMessage(message: unknown): boolean {
  return !isAssistantSilentReply(message) && !shouldHideSystemToolMessage(message);
}
...
export async function loadChatHistory(state: ChatState) {
  ...
  try {
    ...
    state.chatMessages = messages.filter(shouldRenderMessage);
...
export function handleChatEvent(state: ChatState, payload?: ChatEventPayload) {
  ...
  if (payload.runId && state.chatRunId && payload.runId !== state.chatRunId) {
    if (payload.state === "final") {
      ...
      if (finalMessage && shouldRenderMessage(finalMessage)) {
  ...
  if (payload.state === "delta") {
    ...
    if (
      ...
      !shouldHideSystemToolStreamText(next)
    ) {
    ...
    if (finalMessage && shouldRenderMessage(finalMessage)) {
      ...
      !shouldHideSystemToolStreamText(state.chatStream)
    ) {
      state.chatMessages = [
        ...state.chatMessages,
        {
    ...
    if (normalizedMessage && shouldRenderMessage(normalizedMessage)) {
      ...
      if (
        ...
        !shouldHideSystemToolStreamText(streamedText)
      ) {
        state.chatMessages = [
          ...state.chatMessages,
          {
