// 复制自 openclaw 3.13 原版 ../../../ui/src/ui/chat/message-normalizer.ts，用于二开定制
...
import { resolveAvatarImage, resolveInitialAvatarText } from "../../../../utils/groupSurface.ts";
  ...
  const senderLabel =
    typeof m.senderLabel === "string" && m.senderLabel.trim()
      ...
      : typeof m.senderName === "string" && m.senderName.trim()
        ? m.senderName.trim()
        : null;
  const senderId = typeof m.senderId === "string" && m.senderId.trim() ? m.senderId.trim() : null;
  const senderAvatarUrl = resolveAvatarImage({
    avatarUrl: m.senderAvatarUrl,
    avatar: m.senderAvatar ?? m.avatar,
    image: m.senderImage ?? m.image,
  });
  const senderAvatarText = senderLabel ? resolveInitialAvatarText(senderLabel) : null;
