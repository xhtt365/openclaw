...
- and a growing set of original source files recovered from local Codex session logs under `~/.codex/sessions/` plus local unreachable Git WIP/index objects.
...
Recovered into the tree so far:
...
- state, services, types, and utils: `src/stores/agentStore.ts`, `src/stores/chatStore.ts`, `src/stores/groupStore.ts`, `src/stores/officeStore.ts`, `src/services/gateway.ts`, `src/services/gateway.test.ts`, `src/types/agent.ts`, `src/types/config.ts`, `src/types/gateway.ts`, `src/types/model.ts`, `src/utils/*`, `src/constants/agentTemplates.ts`
...
`src/stores/agentStore.ts` has now been restored from a local Git WIP/index object, but the repository intentionally keeps the recovery shell as the active entrypoint instead of wiring the restored app back into `main.tsx` yet.
